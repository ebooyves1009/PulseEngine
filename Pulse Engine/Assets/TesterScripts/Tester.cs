...
public class Tester : MonoBehaviour, IMovable
{
    ...
    private async Task Update()
    {
        if (currentCam)
        {
            ...
            {
                PulseDebug.DrawCircle(hit.point, 1, hit.normal, Color.green);
        ...
        //Display path grid
        {
            ...
            try
            {
                ...
                for (int i = 0; i < ckhArray.Length; i++)
                {
                    ...
                    if (node == startNode || node == endNode)
                    {
                        PulseDebug.DrawCircle(pos, cell_Size * 0.5f, Vector3.up, Color.cyan, 90);
                    }
                    else if (ClosedList.Contains(node))
                        PulseDebug.DrawCircle(pos, cell_Size * 0.5f, Vector3.up, Color.red, 90);
                    else if (OpenList.Contains(node))
                    {
                        if (node == OpenList[0])
                            PulseDebug.DrawCircle(pos, cell_Size * 0.5f, Vector3.up, Color.magenta, 90);
                        else
                            PulseDebug.DrawCircle(pos, cell_Size * 0.5f, Vector3.up, Color.green, 90);
                    }
                    else
                    {
                        PulseDebug.DrawCircle(pos, cell_Size * 0.5f, Vector3.up, ckhArray[i] >= 0 ? Color.blue : (node.Walkable ? Color.gray : Color.black), 90);
                        //PulseDebug.Draw2dPolygon(pos, cell_Size * 0.5f, Vector3.up, Color.yellow, 60);
                    }
                }
            }
    ...
    private void CheckPhysicSurround(LayerMask mask, float checkDist = 1, JobHandle dependancy = default(JobHandle))
    {
        ...
        for (int i = 0; i < envHits.Length; i++)
        {
            ...
            if (hit.transform)
                PulseDebug.DrawCircle(hit.point, rayLenght * 0.2f, hit.normal, c);
        }
    }
