...
public class WeaponDataEditor : PulseEditor<WeaponDatas>
{
    ...
    [MenuItem(Menu_EDITOR_MENU + "/Combat System/Weapon Editor")]
    ...
    private void Initialisation()
    {
        ...
        for (int i = 0; i < 100; i++)
        {
            var asset = AssetDatabase.LoadAssetAtPath<WeaponDatas>(GAME_RES_PATH + Save_Path + "/" + SaveFileName + "_" + i + ".asset");
            if (asset != null)
            {
                dataList.Add(asset);
            }
    ...
    private void OverrideControllerList(WeaponDatas data)
    {
        ...
        GroupGUI(() =>
        {
            ...
            WriteField<WeaponDatas>("_animatorOverrideController", data, ovController);
    ...
    private void GlobalParameters(WeaponDatas data)
    {
        ...
        GroupGUI(() =>
        {
            ...
            WriteField<WeaponDatas>("_restBone", data, bone);
            ...
            WriteField<WeaponDatas>("_restParams", data, new TransformParams { position = pos, rotation = rot, scale = sca });
        }, "Rest Params", 160);
        GroupGUI(() =>
        {
            var bone = (HumanBodyBones)(EditorGUILayout.EnumPopup("Equip Bone", data.EquipBone));
            WriteField<WeaponDatas>("_equipBone", data, bone);
            ...
            WriteField<WeaponDatas>("_equipParams", data, new TransformParams { position = pos, rotation = rot, scale = sca });
        }, "Equip Params", 160);
    ...
    private void BasicInfos(WeaponDatas data)
    {
        ...
        GroupGUI(() =>
        {
            ...
            WriteField<BaseSerializedData>("_name", data, name);
            string desc = EditorGUILayout.TextField("Description", data.Description);
            WriteField<BaseSerializedData>("_description", data, desc);
        }, "Basic Infos", 120);
    }
    ...
    private void OverrideParams(WeaponDatas data)
    {
        ...
        GroupGUI(() =>
        {
            ...
            var prevWeapon = (GameObject)EditorGUILayout.ObjectField("Weapon Preview", weaponPreview, typeof(GameObject), false);
            if (prevWeapon != weaponPreview)
            {
                weaponPreview = prevWeapon;
                ResetPreviews();
            }
            PreviewAnAnimation(Ov_anim, 32 / 9, null, _previewWeaponRestPlace
                ? (weaponPreview, data.RestBone, data.RestParams.position, data.RestParams.rotation)
                : (weaponPreview, data.EquipBone, data.EquipParams.position, data.EquipParams.rotation));
            EditorGUILayout.BeginHorizontal();
        ...
        GroupGUI(() =>
        {
            if (data.Attacks == null)
                WriteField<WeaponDatas>("_attacks", data, new List<AttackParams>());
            if (data.Attacks.TryGetValue(ov_baseName, out var atkValue))
            {
                ...
                WriteField<AttackParams>("_physicSpace", atkValue, space);
                EditorGUILayout.EndVertical();


                //hit frames
                if (atkValue.HitFrames == null)
                    WriteField<AttackParams>("_hitFrames", atkValue, new List<HItFrame>());
                ...
                WriteField<AttackParams>("_hitFrames", atkValue, atkValue.HitFrames);
                EditorGUILayout.EndFoldoutHeaderGroup();


                //Chain phases
                if (atkValue.ChainsPhases == null)
                    WriteField<AttackParams>("_chainsPhases", atkValue, new List<ComboChainPhase>());
                ...
                WriteField<AttackParams>("_chainsPhases", atkValue, atkValue.ChainsPhases);
                EditorGUILayout.EndFoldoutHeaderGroup();
            }
            else
            {
                if (GUILayout.Button("Create Attack"))
                {
                    ...
                    WriteField<AttackParams>("_overrideAnimName", atk, ov_baseName);
