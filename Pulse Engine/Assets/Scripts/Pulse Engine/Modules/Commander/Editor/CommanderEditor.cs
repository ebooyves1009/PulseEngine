...
namespace PulseEditor.Modules
{
    ...
    public class CommanderEditor: PulseEditorMgr
    {
        ...
        public static void RefreshCache(object _dictionnary, DataTypes _dtype)
        {
            ...
            {
                if (CoreLibrary.Exist<CommandsLibrary>(AssetsPath, scp))
                {
                    var load = CoreLibrary.Load<CommandsLibrary>(AssetsPath, scp);
                    if (load != null)
                        allAsset.Add(load);
                }
                else if (CoreLibrary.Save<CommandsLibrary>(AssetsPath, scp))
                {
                    var load = CoreLibrary.Load<CommandsLibrary>(AssetsPath, scp);
                    if (load != null)
                        allAsset.Add(load);
